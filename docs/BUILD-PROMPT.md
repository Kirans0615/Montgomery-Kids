# BUILD PROMPT — 4Montgomery's Kids: full website rebuild (static, animated, one pass)

You are building a complete, production-ready, fully static redesign of **4montgomeryskids.org** (a 501(c)(3) nonprofit in Bethesda, MD that helps abused and neglected children and youth in Montgomery County's child welfare system). The finished site must look far more polished and alive than the current WordPress/Divi site. It needs motion, depth, texture, warmth and real craft. It must keep the **same page structure, same navigation and the same wording** as the current site (with the specific fixes listed in §12).

Build the whole thing in this one session: every page, every section, every component, all content, all images wired, SEO, accessibility, a passing production build, and a QA pass. Do not stop at a scaffold. Do not leave TODOs, lorem ipsum, "coming soon" or placeholder copy anywhere.

---

## 0. Inputs you have

1. **Current-site audit (source of truth for ALL copy):** `docs/site-audit.md`, which is the file `4montgomeryskids-site-audit.md` that I'm providing. Copy it into `docs/site-audit.md` before you start. Every word of on-page copy, every story, every testimonial, every board bio, and the full text of all 21 blog posts come from that file. Read the whole file first (it is about 745 lines, so read it in chunks until you reach the end).
2. **Existing images (use these; they are the real site's assets):** GitHub repo `https://github.com/Kirans0615/Montgomery-Kids` (public). Clone it and copy the images into the project (see §4 for the exact mapping and new file names):
   ```bash
   git clone --depth 1 https://github.com/Kirans0615/Montgomery-Kids /tmp/mk-assets
   ```
3. **Donation form (external, keep it external):** `https://secure.4montgomeryskids.org/forms/donations`
4. **Facebook:** `https://www.facebook.com/4montgomeryskids`
5. **Contacts:** President Leslie Shedlin, `lkshedlin@4montgomeryskids.org`. Board Chair Agnes Leshner, `aleshner@4montgomeryskids.org`.
6. **Mailing address (use this everywhere):** 4Montgomery's Kids, P.O. Box 34864, 10421 Motor City Drive, Bethesda, MD 20817

---

## 1. Skills to use (load these in this order; one design philosophy only)

- **Design philosophy (the only one):** `frontend-design`. Use it for the overall aesthetic direction, type and composition, and to avoid templated defaults. Do NOT also load `minimalist-ui`, `industrial-brutalist-ui`, `gpt-taste` or the other taste skills. Mixing philosophies is exactly what we're avoiding.
- **Implementation:** `ui-styling` (shadcn/ui + Tailwind, accessible primitives) and `rampstack-skills:frontend-component-build`.
- **Reference as needed:** `ui-ux-pro-max` (font pairing and palette sanity check only; the palette and fonts below are already decided, so don't replace them).
- **Charts:** `dataviz` for the grant-allocation donut and the stat tiles.
- **Copy:** `rampstack-skills:editorial-qa`, used ONLY to apply the typo fixes in §12. Do not rewrite copy. `rampstack-skills:landing-page-copy` may be used ONLY for new microcopy on NEW sections (labels, eyebrows, aria text, empty states), and it must stay in the org's plain, warm, unpretentious voice.
- **SEO:** `rampstack-skills:seo-onpage` + `rampstack-skills:seo-technical` (meta, OG, JSON-LD, sitemap, robots, redirects).
- **QA at the end:** `run` (launch and click through every page at 390px, 768px and 1440px), `rampstack-skills:accessibility-audit`, `web-perf`, `rampstack-skills:qa-testing`.
...
src/
  ...
  routes/            Home.tsx, About.tsx, Stories.tsx, Programs.tsx, Blog.tsx, Post.tsx, NotFound.tsx
  components/
    layout/          Navbar.tsx, MobileMenu.tsx, Footer.tsx, PageTransition.tsx, SmoothScroll.tsx, StickyDonateBar.tsx, Seo.tsx
    sections/        (one file per section, named by page, e.g. HomeHero.tsx, HomeStoriesSlider.tsx, ...)
    ui/              shadcn primitives + display-cards.tsx + hand-writing-text.tsx
    motion/          Reveal.tsx, SplitText.tsx, Counter.tsx, Marquee.tsx, Parallax.tsx, MagneticButton.tsx, TiltCard.tsx
  hooks/             useVideoFade.ts, useScrolled.ts, useReducedMotion.ts, useLockBody.ts
  data/              site.ts, stories.ts, testimonials.ts, board.ts, partners.ts, programs.ts, posts.ts, timeline.ts, allocation.ts
  lib/               utils.ts, donate.ts (confetti + open donation form)
public/
  images/            (originals, renamed) + images/opt/ (generated webp)
  og/                og-default.jpg (1200×630 composed from hero image + logo)
  favicon.png, robots.txt, sitemap.xml, _redirects
